              ...
              <div className="space-y-1.5">
                {markets.slice(0, MAX_ITEMS).map((m) => (
                  <Link
                    key={m.slug}
                    href={`/markets/${m.slug}`}
                    className="flex items-center gap-2 rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2 transition hover:border-white/15 hover:bg-white/[0.05]"
                  >
              ...
              <div className="flex flex-wrap gap-1.5">
                {tickers.slice(0, 8).map((t) => (
                  <Link
                    ...
                    className="rounded-lg border border-cyan-500/20 bg-cyan-500/5 px-2.5 py-1 text-[11px] font-semibold text-cyan-200 transition hover:border-cyan-500/40 hover:bg-cyan-500/10"
                  >
                    ${t.symbol}
                  </Link>
                ))}
